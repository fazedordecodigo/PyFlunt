...
class StringValidationContract(Notifiable):
	...
	def is_not_none_or_white_space(self, value: str, field: str, message: str = IS_NOT_NONE_OR_WHITESPACE) -> Self:
		"""
	...
	def contains(
		...
		if value.find(comparer) == -1:
			self.add_notification(Notification(field, message.format(field, comparer)))
			return self
